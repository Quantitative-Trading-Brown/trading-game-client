...
import { orderbook } from "@/utils/Types";

const generateBook = (bookLim: number[]) => {
  return Array.from({ length: bookLim[1] - bookLim[0] + 1 }, (_, i) => ({
    [bookLim[1] - i]: 0,
  })).reduce((acc, curr) => Object.assign(acc, curr), {});
};

type orderbook_props = {
  admin: boolean;
  book: orderbook;
  bookLim: number[];
};

const Orderbook = (props: orderbook_props) => {
  ...
  useEffect(() => {
    setOrderbook(generateBook(props.bookLim));
  }, [props.bookLim]);

  useEffect(() => {
    if (socket) {
      socket.on("orderbook", (updates: orderbook) => {
            ...
            {Object.entries(orderbook)
              ...
              .map(([price, vol]: [any, any]) => (
                <tr key={price} className="border-b border-gray-300 h-10">
                  ...
                  <td
                    className={"px-4" + (vol > 0 ? " bg-green-700" : "")}
                  >
                    {Math.max(vol, 0)}
                  </td>
