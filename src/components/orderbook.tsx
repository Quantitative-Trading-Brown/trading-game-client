...
const OrderbookCell = (props: OrderbookProps) => {
  ...
  const handleBidAskQuantityChange = (
    ...
    setBidAskQuantity(Math.max(1, Math.min(10, Number(event.target.value))));
  };
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="overflow-y-auto overflow-x-auto h-[20em]">
        <table className="table-auto border-collapse w-full">
          <thead className="outline outline-1 outline-offset-0 outline-red sticky top-0">
            <tr className="bg-black">
              <th className="text-left px-2 py-2">Bids</th>
              <th className="text-left px-2 py-2">Asks</th>
              ...
              ? Object.entries(orderbooks[selectedSecurity])
                  ...
                  .map(([price, vol]: [any, any]) => (
                    <tr key={price} className="border-b border-gray-300 h-10">
                      ...
                      {!props.admin ? (
                        <td className="h-10 p-0 border-x-white border-x">
                          <button
                            onClick={() => handleOrder("BUY", price)}
                            className="bg-gray-500 h-full w-full px-4
      ...
      <div className="p-5 min-h-[250px] flex-grow">
